...
export const FeedbackPanel: React.FC<FeedbackPanelProps> = ({
	...
	let countSeverityClass: string;
	if (errorCount > 0) {
		countSeverityClass = styles.error;
	} else if (warningCount > 0) {
		countSeverityClass = styles.warning;
	} else {
		countSeverityClass = styles.success;
	}
		...
		<div className={styles.feedbackPanel}>
			...
			<div className={styles.diagnosticsList}>
				{diagnostics.map((diagnostic, index) => {
					...
					const isExpanded = expandedDiagnostics.has(
						`${diagnostic.ruleId}-${index}`,
					);
			...
			<div className={styles.footer}>
				<div className={styles.summary}>
					<span
						className={`${styles.summaryItem} ${errorCount > 0 ? styles.error : ""}`}
					>
						{errorCount} errors
					</span>
					<span
						className={`${styles.summaryItem} ${warningCount > 0 ? styles.warning : ""}`}
					>
						{warningCount} warnings
					</span>
					<span
						className={`${styles.summaryItem} ${infoCount > 0 ? styles.info : ""}`}
					>
						{infoCount} info
					</span>
				</div>
