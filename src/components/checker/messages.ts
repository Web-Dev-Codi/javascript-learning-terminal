...
export class MessageDictionary {
  ...
  static {
    ...
    this.messages.set('require-assignment-operator', {
      ...
      long: "Variable '${tokenName}' must include an assignment operator (=) and a value",
      hint: "Add an assignment operator (=) followed by a value for '${tokenName}'"
    })
    ...
    this.messages.set('const-reassignment', {
      ...
      long: "Variables declared with const cannot be reassigned ('${tokenName}')",
      hint: "Use let instead of const if you need to reassign '${tokenName}'"
    })

    this.messages.set('undefined-variable', {
      short: 'Undefined variable',
      long: "Variable '${tokenName}' is being used but has not been declared",
      hint: "Declare '${tokenName}' before using it with let or const"
    })
    ...
    this.messages.set('runtime-error', {
      short: 'Runtime error',
      long: 'Your code threw an error during execution',
    })
  }
  ...
  static formatDiagnostic(diagnostic: Diagnostic): Diagnostic {
    ...
    return {
      ...
      messages: {
        ...
        long: diagnostic.messages.long || formatString(template.long),
        hint: diagnostic.messages.hint || (template.hint ? formatString(template.hint) : undefined),
      }
    }
  }
  ...
  static getMessage(ruleId: string, context?: string): string {
    ...
    if (!template) return `Unknown rule: ${ruleId}`
