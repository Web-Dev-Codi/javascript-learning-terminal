...
const rules: Map<RuleId, Rule> = new Map();
...
export function walkAST(
	ast: ESTreeAST,
	visitors: Record<string, (node: ESTreeNode) => void>,
): void {
			...
			VariableDeclaration: (node: ESTreeNode) => {
				...
				if (declarations) {
					for (const declarator of declarations) {
						...
						if (!declarator.init && id?.loc && id.name) {
							diagnostics.push({
...
const undefinedVariableRule: Rule = {
	...
	check: ({ ast }: { ast: ESTreeAST; source: string }): Diagnostic[] => {
		...
		const processChild = (
			child: unknown,
			parent: ESTreeNode,
			buildParentMap: (node: ESTreeNode, parent: ESTreeNode | null) => void,
		) => {
			if (Array.isArray(child)) {
				for (const item of child) {
					if (item && typeof item === "object")
						buildParentMap(item as ESTreeNode, parent);
				}
			} else if (
				child &&
				typeof child === "object" &&
				(child as ESTreeNode).type
			) {
				buildParentMap(child as ESTreeNode, parent);
			}
		};
		...
		const buildParentMap = (node: ESTreeNode, parent: ESTreeNode | null) => {
			...
			for (const key in node) {
				...
				processChild(child, node, buildParentMap);
			}
		};
		...
		walkAST(ast, {
			...
			CatchClause: (node: ESTreeNode) => {
				const param = node.param as
					| (ESTreeNode & { name?: string })
					| undefined;
		...
		walkAST(ast, {
			Identifier: (node: ESTreeNode) => {
				...
				if (
					...
					(parent as ESTreeNode & { property?: ESTreeNode }).property ===
						node &&
					!(parent as ESTreeNode & { computed?: boolean }).computed
				) {
					return;
				}
				...
				if (
					...
					(parent as ESTreeNode & { key?: ESTreeNode }).key === node &&
					!(parent as ESTreeNode & { computed?: boolean }).computed
				) {
					return;
				}
				...
				if (
					parent?.type === "Property" &&
					(parent as ESTreeNode & { shorthand?: boolean }).shorthand
				) {
					return;
				}
...
function handleIdentifier(pattern: ESTreeNode, set: Set<string>): boolean {
...
function handleObjectPattern(pattern: ESTreeNode, set: Set<string>): boolean {
	const properties = pattern.properties as ESTreeNode[] | undefined;
	if (!properties) return false;

	for (const prop of properties) {
		const propType = (prop as ESTreeNode & { type?: string }).type;
...
function handleArrayPattern(pattern: ESTreeNode, set: Set<string>): boolean {
...
function handleArgumentPattern(pattern: ESTreeNode, set: Set<string>): boolean {
	const arg = (pattern as ESTreeNode & { argument?: ESTreeNode }).argument;
	if (arg) {
		collectBindingNames(arg, set);
		return true;
	}
	return false;
}
...
function collectBindingNames(pattern: ESTreeNode, set: Set<string>) {
	...
	switch (pattern.type) {
		case "Identifier":
			handleIdentifier(pattern, set);
			break;
		case "ObjectPattern":
			handleObjectPattern(pattern, set);
			break;
		case "ArrayPattern":
			handleArrayPattern(pattern, set);
			break;
		case "RestElement":
		case "AssignmentPattern":
			handleArgumentPattern(pattern, set);
			break;
	}
}

// Register all rules after their definitions
registerRule(requireSemicolonsRule);
registerRule(requireAssignmentOperatorRule);
registerRule(noVarRule);
registerRule(constReassignmentRule);
registerRule(undefinedVariableRule);
