...
import { type ESTreeAST, runRules } from "./ruleEngine";
...
export const useSyntaxChecker = (): UseSyntaxCheckerResult => {
	...
	const checkSyntax = useCallback(
		(code: string, activeRules?: RuleId[]) => {
			...
			try {
				...
				if (!syntaxResult.success) {
					...
					addConsoleMessage(
						"error",
						`✕ ${syntaxResult.errors.length} syntax error(s) found`,
					);
					return;
				}
				...
				const ruleDiagnostics = runRules(
					syntaxResult.ast as unknown as ESTreeAST,
					code,
					rulesToCheck,
				);
				...
				if (summary.errors === 0 && summary.warnings === 0) {
					...
					if (summary.info > 0) {
						addConsoleMessage(
							"info",
							`○ ${summary.info} info message(s) found`,
						);
					}
				}
			} catch (error) {
