...
export const useCodeMirror = ({
	...
	const highlightExtension = useMemo(() => {
		...
		return StateField.define({
			...
			update(_decorations, tr) {
				...
				const decorations = highlightLines.map((lineNumber) => {
					const line = tr.state.doc.line(Math.max(1, lineNumber));
					return Decoration.line({
						class: "cm-highlight-line",
					}).range(line.from);
				});
	...
	const setEditorDiagnostics = useCallback((diagnostics: AppDiagnostic[]) => {
		if (!editorRef.current) return;
		const view = editorRef.current;

		const lintDiagnostics: LintDiagnostic[] = diagnostics.map((diagnostic) => {
			const line = view.state.doc.line(Math.max(1, diagnostic.line));
		...
		const transaction = setLintDiagnostics(view.state, lintDiagnostics);
